 ...
 * - the data cant be changed once its set 
 * - fixed aspect ratio 
 * - the content that is being drawn is set in the constructor
 * 
			...
			for (int j = 0; j < data[0].length; j++) {
				...
				if (data[i][j] == null) {
					throw new RuntimeException(
							"the array element cannot be null");
				}
	...
	public String toString() {
		...
		for (int i = 0; i < data.length; i++) {
			for (int k = 0; k < Math.ceil(maxHeight); k++) {
				for (int j = 0; j < data[0].length && data[i][j] != null; j++) {
					line = PictHelper.getLine(data[i][j].toString(), k);
					ret.append(line);
		...
		if (scale < 1.0) {
			...
			for (int i = 0; i < lastHeight; i++) {
				ret2.append(PictHelper.getLine(text, i).substring(0, lastWidth) + "\n");
			}
			...
			for (int i = 0; i < lastHeight; i++) {
				...
				String newLine = PictHelper.getLine(text, i % numLines);
